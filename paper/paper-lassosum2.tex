...
Also note that, as in LDpred2, we run lassosum2 genome-wide and do not require splitting the genome into independent LD blocks anymore.
...
In terms of computational speed, lassosum2 runs in a few minutes compared to a few hours for lassosum and LDpred2.
...
Averaged over 10 simulations, when providing true per-variant GWAS sample sizes, squared correlations (in \%) between the polygenic scores and the simulate phenotypes are of 13.9 for C+T, 15.4 for PRS-CS, 17.5 for lassosum, 18.0 for lassosum2, and 17.8 for LDpred2 (Figure \ref{fig:simu-misN}).
...
For methods that use the per-variant GWAS sample size, predictive performance slightly decreased from 17.5 to 17.4 for lassosum, from 18.0 to 17.5 for lassosum2, but decreased by a lot for LDpred2 with new values of 11.5 for LDpred2-grid, 5.7 for LDpred2-auto and 6.8 for LDpred2-inf (Figure \ref{fig:simu-misN}).
...
To sum up, here we propose a new version of lassosum, lassosum2, that can be run with almost no extra coding nor computational time when already running LDpred2 because it is very fast and uses the same input arguments as LDpred2.
We also show that it generally provides good predictive performance, sometimes better than LDpred2, e.g.\ in the case of severe sample size misspecification.
Finally, we want to note that, except from the GWAS summary statistics provided by the Psychiatric Genomics Consortium (PGC), per-variant sample sizes are often not reported in GWAS summary statistics.
We show here that lassosum is robust to this misspecification, but that it can dramatically affect predictive performance of LDpred2; we may wonder which other statistical genetics methods could be affected by this missing information?
Although that this sample size misspecificaition can probably be detected from the quality control plot we propose in \cite{prive2020ldpred2} (Figure \ref{fig:qcplot}), we urge researchers to report the per-variant sample sizes from their GWAS meta-analyses.
